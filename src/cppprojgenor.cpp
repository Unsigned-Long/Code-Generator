...
void handleParams(int argc, char const *argv[]) {
  if (argc == 1) {
    std::cout << "\nThis simple program is used to generate cpp project "
                 "structure.\n\n";
...
void genor(const std::string &projpath) {
  ...
  writeContext(FULLPATH(projpath, ".gitignore"),
               "/build\n"
               "/.gitignore");
  ...
  writeContext(cmakefile,
               ...
               "${CMAKE_SOURCE_DIR}/main.cpp ${SRC_FILES})\n\n");
  // bash files
  ...
  writeContext(FULLPATH(projpath, "bash/makerun.sh"),
               "cmake --build ../build/\n"
               "cd ../bin\n"
               "./" +
                   projname + "\ncd ../bash");
