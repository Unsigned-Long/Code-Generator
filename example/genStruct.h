...
public:
     ...
     */
    Car(const std::string &provider, const float &price, const double &weight, const unsigned char &color)
...
{
    os << '{';
    os << "'provider': " << obj.provider() << ", 'price': " << obj.price() << ", 'weight': " << obj.weight() << ", 'color': " << obj.color();
    os << '}';
    return os;
}